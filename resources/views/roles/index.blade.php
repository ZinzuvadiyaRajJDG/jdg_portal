...
@section('content')
<style>
    /* Add your desired background color and border style */
    .badge {
...
<h2 class="intro-y text-lg font-medium mt-10">
        ...
        @can('role-create')
        <a class="btn btn-primary shadow-md mr-2" href="{{ route('roles.create') }}">Add Roles</a>
        @endcan
                        {{-- <div class="dropdown">
                    ...
                    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                        <table class="table table-report -mt-2">
                            ...
                            <tbody>
                                ...
                                <tr class="intro-x">
                                    <td>
                                        <a href="" class="font-medium whitespace-nowrap">{{ $role->id }}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @foreach($role->permissions as $permission)
                                            <span class="badge rounded-pill badge-primary mb-1">{{ $permission->name }}</span>
                                        @endforeach                                  
                                    </td>
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            <a class="flex items-center mr-3" href="{{ route('roles.show',$role->id) }}"><i data-feather="eye" class="w-4 h-4 mr-1"></i>Show</a>
                                            <a class="flex items-center mr-3" href="{{ route('roles.edit',$role->id) }}"><i data-feather="check-square" class="w-4 h-4 mr-1"></i>Edit</a>
                                            <a class="flex items-center text-theme-6" href="javascript:;" data-toggle="modal" data-target="#delete-confirmation-modal{{ $role->id }}"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete </a>
                                        </div>
                                    </td>
                                </tr>
