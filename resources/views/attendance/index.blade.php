        ...
        <table class="table table-report -mt-2">
            ...
            <tbody>
                @foreach ($attendancesAdmin as $attendanceAdmin)
                    @if (!empty($attendanceAdmin->user->getRoleNames()))
                        @foreach ($attendanceAdmin->user->getRoleNames() as $role)
                            @php $role_name =  $role; @endphp
                        @endforeach
                    @endif
                    <!-- Use a form for each attendance row -->
                    @can('management-team-attendance')
                            ...
                            <tr class="intro-x">
                                ...
                                <td class="table-report__action w-56">
                                    <div class="flex justify-center items-center">
                                        ...
                                        @if($role_name ==  Auth::user()->roles->first()->name && $attendanceAdmin->user_id == Auth::user()->id)
                                        @else

                                            <button type="submit" name="status" value="half day" class="btn btn-sm w-20 mr-1 mb-2 {{ $attendanceAdmin->day === 'half day' ? 'active btn-warning' : 'btn-outline-warning' }}">Half Day</button>
                                            @if($attendanceAdmin->status == 'paid holiday' && $attendanceAdmin->day == 'paid holiday')
                                                <button type="submit" name="status" value="paid holiday" class="btn btn-sm w-24 mr-1 mb-2 btn-outline-orange">Paid Leave</button>
                                            @else
                                    ...
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            ...
                                            <button type="submit" name="status" value="present"
                                                class="btn btn-sm w-15 mr-1 mb-2 {{ $attendanceAdmin->status === 'present' ? 'active btn-success' : 'btn-outline-success' }}">Present</button>
                                            <button type="submit" name="status" value="absent"
                                                class="btn btn-sm w-15 mr-1 mb-2 {{ $attendanceAdmin->status === 'absent' ? 'active btn-danger' : 'btn-outline-danger' }}">Absent</button>
                                            @if($role_name == Auth::user()->roles->first()->name)
                                            @else
                                            <button type="submit" name="status" value="half day"
                                                class="btn btn-sm w-20 mr-1 mb-2 {{ $attendanceAdmin->day === 'half day' ? 'active btn-warning' : 'btn-outline-warning' }}">Half
                                                Day</button>
                                            @if($attendanceAdmin->status == 'paid holiday' && $attendanceAdmin->day == 'paid
                                            holiday')
                                            <button type="submit" name="status" value="paid holiday"
                                                class="btn btn-sm w-24 mr-1 mb-2 btn-outline-orange">Paid Leave</button>
                                            @else
                                            <button type="submit" name="status" value="paid holiday"
                                                class="btn btn-sm w-24 mr-1 mb-2 {{ $attendanceAdmin->status === 'paid holiday' ? 'active btn-primary' : 'btn-outline-primary' }}">Paid
                                                Holiday</button>
                                            @endif
                                            @endif
                                        </div>
                    ...
                    <tr class="intro-x">
                        ...
                        <td class="table-report__action w-56 text-center">
                            @php
                                ...
                                {
                                    $statusClass = 'btn-rounded-orange';
                                }
                                else
                                {
