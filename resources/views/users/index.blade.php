...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 sm:col-span-6 flex flex-wrap sm:flex-nowrap items-end">
            @can('employee-create')
                <a class="btn btn-primary shadow-md mr-2 mb-3" href="{{ route('users.create') }}">Create New User</a>
            @endcan
        </div>
        <div
                ...
                <tbody>
                    @if (count($data) > 0)
                        @foreach ($data as $user)
                                ...
                                <tr class="intro-x">
                                    ...
                                    <td class=" whitespace-nowrap">
                                        @if ($user->status == 'Active')
                                            <a class="flex items-center text-theme-6" href="javascript:;" data-toggle="modal" data-target="#status-change-confirmation-modal{{ $user->id }}">
                                        ...
                                        @else
                                            <a class="flex items-center text-theme-6" href="javascript:;" data-toggle="modal" data-target="#status-change-confirmation-modal{{ $user->id }}">
                                                <button class="flex items-center lg:justify-center text-theme-6" type="submit">
                                    ...
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            @if($role_name == "Management Team")
                                                @can('team-show')
                                                    <a class="flex items-center mr-3" href="{{ route('users.show', $user->id) }}">
                            ...
                            @else
                                @if ($role_name != Auth::user()->roles->first()->name || $user->id == Auth::user()->id)
                                    <tr class="intro-x">
                                        <td>
                                            <a href="{{ route('users.show', $user->id) }}"
                                                class="font-medium whitespace-nowrap">
                                        ...
                                        <td class=" whitespace-nowrap">
                                            @if ($user->status == 'Active')
                                            <a class="text-theme-6" href="javascript:;" data-toggle="modal" data-target="#status-change-confirmation-modal{{ $user->id }}">
                                                <button class="flex items-center lg:justify-center text-theme-9">
                                                    <i data-feather="check" class="w-4 h-4 mr-1"></i> Active
                                                </button>
                                            </a>
                            ...
                            @endcan
                                ...
                                <div id="status-change-confirmation-modal{{ $user->id }}" class="modal" tabindex="-1"
                                    aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-body p-0">
